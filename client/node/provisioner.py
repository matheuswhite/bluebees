...
from common.utils import order
...
class Provisioner(Client):
    ...
    async def _close_link(self, reason: bytes):
        msg_type = b'prov_s'
        content = self.prov_ctx.device_link
        content += self.prov_ctx.client_tr_number.to_bytes(1, 'big')
        content += b'\x0b'
    ...
    def _check_public_key_pdu(self, content) -> bool:
        node_public_key_x = content[1:33]
        node_public_key_y = content[33:65]
    ...
    async def _provisioning_device(self):
        ...
        # link open phase
        for try_ in self.link_open_tries:
            ...
            try:
                ...
                await wait_for(self._wait_link_ack(), self.timeout)
