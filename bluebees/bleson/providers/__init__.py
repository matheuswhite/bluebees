...
def get_provider():
    ...
    if _provider is None:
        if sys.platform.startswith('linux'):
            from bluebees.bleson.providers.linux.linux_provider import LinuxProvider
            _provider = LinuxProvider()
        elif sys.platform.startswith('darwin'):
            from bluebees.bleson.providers.macos.macos_provider import MacOSProvider
            _provider = MacOSProvider()

        elif sys.platform.startswith('win32'):
            from bluebees.bleson.providers.win32.win32_provider import Win32Provider
